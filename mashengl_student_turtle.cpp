...
turtleMove studentTurtleStep(bool bumped_wall, bool at_goal) {
    ...
    // Static variables to control movement sequencing
    static bool just_turned = false;
    ...
    switch (robot_state) {
        ...
        case PLAN_NEXT:
            if (just_turned) {
                // Skip a tick after turning to maintain separation
                just_turned = false;
                ready_to_move = true;
                next_move.validAction = false;
                break;
            ...
            if (best_direction == -1) {
                ...
                if (rotations_checked < 3) {
                    ...
                    // Reset for movement phase
                    rotations_checked = 0;
                    ready_to_move = true;
                    next_move.validAction = false;
                }
            } else {
                // Have best direction, need to move or turn toward it
                if (best_direction == facing_direction) {
                    if (ready_to_move && !bumped_wall) {
                        // Move forward
                        coordinate next_pos = getNextPosition(current_pos, facing_direction);
                        current_pos = next_pos;
                        updateVisitMap(current_pos);
